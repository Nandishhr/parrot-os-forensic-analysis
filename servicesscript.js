...
function runLoginScript() {
    ...
    if (isLoginRunning == false) {
        ...
        lfservice = "true";
        updateData();
        exec(`python3 loginstart.py ${slff} ${slfd} ${userEmail}`, (error, stdout, stderr) => {
            if (error) {
...
function runPassScript() {
    ...
    if (isPassRunning == false) {
        ...
        pcservice = "true";
        updateData();
        exec(`python3 passstart.py ${spcf} ${spcd} ${userEmail}`, (error, stdout, stderr) => {
            if (error) {
                console.error(`Error executing Python script: ${error}`);
                return;
            }
...
function runMalwareScript() {
    ...
    if (isMalwareRunning == false) {
        ...
        mmservice = "true";
        updateData();
        exec(`python3 malware.py ${smmpath} ${userEmail}`, (error, stdout, stderr) => {
            if (error) {
                console.error(`Error executing Python script: ${error}`);
                return;
            }
...
function stopLoginScript() {
    ...
    lfservice = "false";
    updateData();
    exec('sudo pkill -f loginstart', (error, stdout, stderr) => {
        if (error) {
...
function stopPassScript() {
    ...
    pcservice = "false";
    updateData();
    exec('sudo pkill -f passstart', (error, stdout, stderr) => {
        if (error) {
            console.error(`Error stopping Python script: ${error}`);
            return;
        }
...
function stopMalwareScript() {
    ...
    mmservice = "false";
    updateData();
    exec('sudo pkill -f malware', (error, stdout, stderr) => {
        if (error) {
            console.error(`Error stopping Python script: ${error}`);
            return;
        }
